...
import com.itwill.test.RdbmsDao;
...
public class BoardDao extends RdbmsDao {
	...
	public int create(Board board) {
		...
		try {
			conn = getConnection();
			String sql = "INSERT INTO Board1 " + "(boardno, title, writer, content, groupno, step)"
	...
	public int createReply(Board board) {
		...
		try {
			...
			conn = getConnection();
			String sql = "UPDATE board1 " + "SET step = step + 1 " + "WHERE step > ? AND groupno = ?";
			...
			// 댓글 삽입
			sql = "INSERT INTO board1 " + "(boardno, title, writer, content, " + "groupno, step, depth) "
	...
	public int update(Board board) {
		...
		try {
			conn = getConnection();
			String sql = "UPDATE board1 " + "SET title = ?, content = ? ,writer = ?" + "WHERE boardno = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, board.getTitle());
	...
	public void increaseReadCount(int number) {
		...
		try {
			conn = getConnection();
			String sql = "UPDATE board1 " + "SET readcount = readcount + 1 " + "WHERE boardno = ?";
			pstmt = conn.prepareStatement(sql);
