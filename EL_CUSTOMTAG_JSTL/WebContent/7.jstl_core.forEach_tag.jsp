...
<body>
	...
	<ul>
		...
		<c:forEach var="guest" items="${guestList}" varStatus="status"
			begin="1" end="${guestList.size()}" step="1">
			<li>${status.index}--> ${guest.guest_no},${guest.guest_name}</li>
		</c:forEach>
		<li>---------Map--------------</li>
		<c:forEach var="guestEntry" items="${guestMap}">
			<li>${guestEntry.key}:${guestEntry.value.guest_no},
				${guestEntry.value.guest_name}</li>
		</c:forEach>
		<c:forEach var="guestEntry" items="${guestMap}" varStatus="status">
